...
+(NSString*) JSONValueOfObject:(id)object forKey:(NSString *)key {
	JSONEncoder* theEncoder = [JSONEncoder encoder];
	[theEncoder encodeObject:object forKey:key assignFinalObject:YES];
...
- (void)encodeObject:(id)object forKey:(NSString *)key {
	[self encodeObject:object forKey:key assignFinalObject:NO];
}

- (void)encodeObject:(id)object forKey:(NSString *)key assignFinalObject:(BOOL) assignFinalObject{
	...
	if(objectEncoding){
		if([objectEncoding isKindOfClass:[NSArray class]] && [object count] > 0){
//			if ([object count] == 0) {
//				return;
//			}
		...
		if (![self topObject]) {
			if ([object isKindOfClass:[NSArray class]]) {
				NSMutableDictionary* dict = [[NSMutableDictionary alloc] initWithObjects:@[object] forKeys:@[objectKey]];
				[self push:dict];
			} else {
				[self push:object];
			}
...
- (NSObject *) getEncodingFor:(id) object{
	...
	if([self isValidJSONObject:object]){
		...
		if ([object isKindOfClass:[UIImage class]]) {
			return [(NSData *) UIImagePNGRepresentation(object) base64EncodedString];
