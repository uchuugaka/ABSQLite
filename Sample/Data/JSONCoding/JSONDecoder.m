...
+(id) decodeWithJSON:(NSString *)json {
	return [JSONDecoder decodeWithData:[json dataUsingEncoding:NSUTF8StringEncoding]];
}
...
+ (id) decodeWithData:(NSData *)json {
	...
	// see if this is an array, and if it is if any entries are dictionaries that can also be decoded.
	// useful for when the top-level array is not populated with a single class type
	if ([decodedObject isKindOfClass:[NSArray class]] ) {
		NSMutableArray* newObjects = [[NSMutableArray alloc] init];
		for (id itemObject in decodedObject) {
			if ([itemObject isKindOfClass:[NSDictionary class]]) {
				decoder = [[JSONDecoder alloc] initWithDictionary:itemObject];
				[newObjects addObject:[decoder decodeObject]];
			} else {
				[newObjects addObject:itemObject];
			}
		}
		
		decodedObject = newObjects;
	}
...
- (id)decodeObject {
	if ([[self topJsonObjectId] isEqualToString:@"root"] && [[self topJsonObject] isKindOfClass:[NSDictionary class]]) {
...
- (NSArray *)decodeArrayOfClass:(Class)class{
	...
	if([[self topJsonObject] isKindOfClass:[NSArray class]]){
		...
		if([[innerDictionary allKeys] count] > 0 || (class == nil && [self isObjectForUnknownClass:innerDictionary])){
